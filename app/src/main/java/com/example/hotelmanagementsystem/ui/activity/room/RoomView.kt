...
class RoomView : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        update = findViewById(R.id.btnRoomUpdate)
        ...
        val bundle = intent.extras?.getBundle("room_masterss")
        val roomDetails:RoomMasters? = bundle?.getSerializable("room_masterss") as RoomMasters?
        ...
        //Text set as they are in view
        if (roomDetails!=null){
            roomNo.setText(roomDetails.roomNo)
            roomImage.setImageURI(roomDetails.roomPicture?.toUri())
            category.setText(roomDetails.roomCategory)
            price.setText(roomDetails.price)
            status.setText(roomDetails.status)
            guest.setText(roomDetails.forGuest)
            description.setText((roomDetails.roomDescription))
        }else{
        ...
        update.setOnClickListener {
            ...
            val roomDescription = description.text.toString()

            val updateRoomDetails = RoomMasters(
                null,priceInput,categoryInput,roomNumberInput,roomImg,statusInput,roomDescription,guestInput)
            if (allFillings()){
                showLoadingDialogBox()
                roomDetails?.id?.let { it1 -> hotelViewModel.updateRoom(it1,updateRoomDetails) }
                uploadImage()
            }else{
                hideLoadingDialogBox()
